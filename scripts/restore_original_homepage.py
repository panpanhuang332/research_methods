"""Restore the exact public homepage selected by the owner, without fonts or app data."""
...
PINNED_IMAGES={'portal-desktop-1536.avif':'14eee9d1b33f2373d6c6f368792ef096f0d094a046e3738429e5214d29258318','portal-mobile-480.avif':'1c24dba8bd307fec39c06b57946202de302b9033a122da34e7125a180ec78378'}
...
ASSETS.mkdir(parents=True,exist_ok=True);QA.mkdir(exist_ok=True)
...
with sync_playwright() as p:
    ...
    assert response is not None and response.status==200,'Original homepage must be publicly readable.'
    assert page.title()=='研究方法18週學程｜服務與科技管理研究所','Unexpected original homepage.'
    page.evaluate('document.fonts.ready')
    soup=BeautifulSoup(page.content(),'html.parser')
    portal=soup.select_one('main.joint-portal')
    assert portal and portal.select_one('.portal-scene img') and portal.select_one('.portal-modern .modern-gate')
    assert '青燈四峰錄' in ''.join(portal.select_one('#ancient-title').stripped_strings)
    styles=soup.select('link[rel="stylesheet"]');assert len(styles)==1
    css_url=urljoin(SOURCE,styles[0]['href'])
    assert urlparse(css_url).netloc==urlparse(SOURCE).netloc
    css_response=context.request.get(css_url,timeout=30000);assert css_response.status==200
    css_bytes=css_response.body();assert hashlib.sha256(css_bytes).hexdigest()==PINNED_CSS,'Source stylesheet changed; review before importing.'
    def absolute_url(match):
        ...
        return 'url("'+resolved+'")'
    css=re.sub(r'url\(([^)]+)\)',absolute_url,css_bytes.decode('utf-8'))
    css+='\n/* Original-course link preserves the original visual appearance. */\n.access-note .origin-course{text-decoration:none}\n'
    ...
    for element in portal.select('picture source,picture img'):
        ...
        if element.get('srcset'):images.update(Path(urlparse(part.strip().split()[0]).path).name for part in element['srcset'].split(','))
    assert images==EXPECTED,'Do not substitute or regenerate original assets.'
    for name in sorted(images):
        url=urljoin(SOURCE,name)
        result=context.request.get(url,timeout=30000)
        print('Original image response:',name,result.status,result.headers.get('content-type'),flush=True)
        assert result.status==200,f'{name}: HTTP {result.status}, MIME {result.headers.get("content-type")}'
        content=result.body()
        assert 1000<len(content)<1000000,'Unexpected file size: '+name
        # A mislabelled application/octet-stream can still be a valid original image.
        # Decode to verify real content, then retain the exact unmodified source bytes.
        try:
            image=Image.open(BytesIO(content));image.load()
            assert image.width>=400 and image.height>=400
        except Exception as e:raise ValueError(f'{name}: response is not a usable original image; {result.headers.get("content-type")}') from e
        digest=hashlib.sha256(content).hexdigest()
        if name in PINNED_IMAGES:assert digest==PINNED_IMAGES[name],'Reviewed image changed: '+name
    ...
    for width,height in [(1440,900),(1024,900),(768,1024),(430,932),(390,844),(375,812),(320,667)]:
        page.set_viewport_size({'width':width,'height':height});page.wait_for_timeout(250);page.evaluate('document.fonts.ready')
        page.screenshot(path=str(QA/f'source-{width}.png'),full_page=True)
        geometry=page.evaluate('''() => Object.fromEntries(['.joint-header','.portal-ancient','.portal-modern','#ancient-title','#modern-title','.ancient-gate','.modern-gate','.joint-login'].map(s=>{const r=document.querySelector(s).getBoundingClientRect();return[s,{x:r.x,y:r.y,width:r.width,height:r.height}]}))''')
        (QA/f'source-{width}.json').write_text(json.dumps(geometry),encoding='utf-8')
    browser.close()
for tag in list(portal.find_all(['script','iframe','object','embed','form'])):tag.decompose()
...
for element in portal.select('picture source,picture img'):
    ...
    if element.get('srcset'):element['srcset']=', '.join('./assets/original-homepage/'+Path(urlparse(part.strip().split()[0]).path).name+' '+part.strip().split()[1] for part in element['srcset'].split(','))
for link in portal.select('a[href]'):
    href=link['href']
    if href=='/':link['href']='./'
    elif href=='/course':link['href']='./visuals/';link['aria-label']='進入研究方法18週學程圖解教材（GitHub版）'
    elif href in ['/progress','/teacher']:
        link['href']=urljoin(SOURCE,href);link['title']='在原課程網站開啟；資料未移轉至 GitHub';link['rel']='noopener'
    else:assert href=='#modern-entry','Unreviewed navigation: '+href
note=portal.select_one('.access-note');assert note
teacher=note.find('a').extract();note.clear()
original_course=soup.new_tag('a',href=urljoin(SOURCE,'course'));original_course['class']='origin-course';original_course['title']='在原課程網站開啟完整教材、留言與評量';original_course.string='免帳號留言與評量'
note.append(original_course);note.append(' · ');note.append(teacher)
html='''<!doctype html>
<html lang="zh-Hant">
<head>
<meta charset="utf-8">
...
<body>
<!-- Original public homepage restored without application scripts or private data. -->
'''+str(portal)+'''\n</body>\n</html>\n'''
...
'''.replace('SOURCE_URL',SOURCE)
    readme.write_text(text.replace('## 直接開啟18章圖解教材',addition+'## 直接開啟18章圖解教材',1),encoding='utf-8')
